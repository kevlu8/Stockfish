        ...
        if (value + inc > bestValue)
        {
            ...
            if (value + inc > alpha)
            {
                ...
                if (value >= beta)
                {
                    ...
                    orderingHistory[posKey & (UINT_16_HISTORY_SIZE - 1)] << (15 - 5 * ss->moveCount) * depth;
...
void update_all_stats(const Position& pos,
    ...
    int bonus = std::min(116 * depth - 81 - orderingHistory[pos.key() & (UINT_16_HISTORY_SIZE - 1)] / 4, 1515) + 347 * (bestMove == ttMove);
    int malus = std::min(848 * depth - 207 - orderingHistory[pos.key() & (UINT_16_HISTORY_SIZE - 1)] / 4, 2446) - 17 * moveCount;
